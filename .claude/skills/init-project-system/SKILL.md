...
2. Identificare l'account attivo in questa sessione dal valore di `$env:CLAUDE_CONFIG_DIR` di
   processo. La mappatura tra directory `.claude-accountN` ed email reale e specifica di questa
   macchina e non va assunta ne' riportata qui: si verifica a inizio sessione con `/status`,
   come descritto in `rules/git-identity-and-repo.md`. Su una macchina diversa cambiano comunque
   i nomi utente e le associazioni: in ogni caso si riportano solo i percorsi rilevati senza
   inventare o stampare le email.
