...
`<dir>\.claude.json`. Su questa macchina `.claude-account1` risulta verificato sull'account
attivo in questa sessione; la mappatura di `.claude-account2` non e stata verificata in questo
passaggio e va confermata con `/status` prima di assumerla, invece di dedurla dal nome della
directory. Per riportare o cambiare
l'account di una directory si imposta prima il browser su claude.ai sull'account desiderato, e
solo dopo, in una sessione avviata con quel `CLAUDE_CONFIG_DIR`, si eseguono `/logout` e
`/login`, confermando infine con `/status`. Toccare l'altra directory mentre il browser e ancora
sull'account sbagliato la ri-vincolerebbe a sua volta a quello: il browser va sempre allineato
prima di ogni `/login`.
...
`/status` e l'identita git con `git log -1 --format="%an <%ae>"`. Su questo repository, per
esempio, l'identita git configurata e quella personale (`alesop95`) indipendentemente
dall'account Claude Code della sessione corrente.
...
Profilo di lavoro: alias SSH `github-corp`, chiave `id_ed25519_corp`, identita `user.name` e
`user.email` dell'organizzazione di lavoro (dettagli specifici della macchina, non riportati
qui). Non e' il profilo con cui questo repository e' configurato.

Profilo personale: alias SSH `github-personal`, chiave `id_ed25519_personal`, identita
`user.name` alesop95 e `user.email` alessio.sopranzi.95@gmail.com, utente GitHub alesop95. E'
il profilo con cui questo repository e' gia configurato.
